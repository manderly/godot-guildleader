...
func draw_hero_and_button():
	if (global.tradeskills["cooking"].hero):
		$button_staffCraft.text = "Craft"
		#draw the hero
		var heroScene = preload("res://baseEntity.tscn").instance()
		heroScene.set_script(preload("res://hero.gd"))
		heroScene.set_instance_data(global.tradeskills["cooking"].hero) #put data from array into scene 
...
func _process(delta):
	var tradeskill = global.tradeskills["cooking"]
